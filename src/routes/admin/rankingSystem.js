...
const rankingSystem = async (_req, res) => {
  ...
  for (let i = 0; i < laptops.length; i++) {
    const brand_name = laptops[i]['laptop_name'].split(' ')[0];
    ...
    const cpu_total_score = cpu_score(laptops[i]);
    const memory_total_score = memory_score(laptops[i]);
    const storage_total_score = storage_score(laptops[i]);
    const connectivity_total_score = connectivity_score(laptops[i]);
    const multimedia__total_score = multimedia_score(laptops[i]);
    const graphics_total_score = graphics_score(laptops[i]);
...
const cpu_score = (laptop) => {
  ...
  const cpu_name = laptop['processor_name'];
  const cpu_brand = laptop['processor_brand'];
  const cpu_generation = laptop['processor_variant'];
  let score = 0;
  let gen;
  ...
  if (cpu_generation !== undefined) {
    ...
    if (cpu_brand == 'Intel') {
      ...
      if (gen <= 9 && gen >= 6) {
        ...
        const new_gen = cpu_generation.substring(0, 2);
...
const memory_score = (laptop) => {
  const ram_type = laptop['ram_type'];
  const ram_size = laptop['ram'];
  const ram_freq = laptop['ram_frequency'];
  let score = 0;
...
const connectivity_score = (laptop) => {
  ...
  if (wifi_name !== undefined && bluetooth_version !== undefined) {
    const w = wifi[wifi_name];
    const b = bluetooth[bluetooth_version];
    if (w === undefined) {
...
const multimedia_score = (laptop) => {
  const size = laptop['screen_size'];
  const resolution = laptop['screen_resolution'];
  let score = 0;
  if (size !== undefined && resolution !== undefined) {
    const s = screen_size[size];
    const r = screen_resolution[resolution];
    if (s !== undefined) {
...
const graphics_score = (laptop) => {
  const graphics = laptop['graphic_processor'].toLowerCase();
